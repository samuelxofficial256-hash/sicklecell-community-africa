    ...
    try {
      ...
      if (!apiKey || apiKey === 'MY_GEMINI_API_KEY' || apiKey.trim() === '') {
        return res.status(200).json({
          ...
          The SCCA AI Assistant is ready! To start chatting, please add your **GEMINI_API_KEY** in the **Settings > Secrets** panel in the top-right of your screen. This key is securely handled by our server proxy to keep your credentials safe from the browser.
...
7. Craft readable responses with bullet points and bolding, using compassionate language that respects African community context. Avoid clinical jargon without explaining it first.`;
  ...
  // Vite static middleware and routes Integration
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[SCCA Server] Full-Stack Server active on http://0.0.0.0:${PORT}`);
  });
}
